        ...
        stage('Setup Worker Nodes') {
            steps {
                withAWS(credentials: 'aws-credentials', region: 'eu-north-1') {
                    ...
                    sh 'aws cloudformation deploy --template-file templates/nodegroup.yml --stack-name eks-node-group --parameter-overrides EKSClusterName=JenkinsApp, --capabilities CAPABILITY_IAM'
